...
export class ProductsComponent {
  ...
  toggleLike(product: any) {
    if (this.likedProducts.has(product.name)) {
      this.likedProducts.delete(product.name);
      this.likeCount--;
    } else {
      this.likedProducts.add(product.name);
      this.likeCount++;
    }
  }

  isLiked(product: any): boolean {
    return this.likedProducts.has(product.name);
  }

  categories: string[] = ['Все', 'Гаджеты', 'Досуг', 'Красота', 'Бытовая техника']; 
  ...
  share(productLink: string) {
    const message = `Посмотри этот товар: ${productLink}`;
    window.open(`https://t.me/share/url?url=${productLink}&text=${message}`, '_blank');
  }
}
